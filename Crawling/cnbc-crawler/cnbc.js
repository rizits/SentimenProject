        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    ...
                    url = "https://www.cnbcindonesia.com/search?query=".concat(encodeURIComponent(keyword), "&page=").concat(pageNumber);
                    console.log("Scraping URL: ".concat(url));
                    return [4 /*yield*/, axios_1.default.get(url)];
                case 3:
                    data = (_b.sent()).data;
                    $ = cheerio.load(data);
                    articleElements = $('div.nhl-list');
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    content = $_1('div.detail-text p').map(function (i, el) { return $_1(el).text().trim(); }).get().join(' ');
                    author = $_1('div.detail-head div.mb-1.text-base.font-semibold').filter(function () {
                        return $_1(this).find('span').length > 0;
                    }).contents().first().text().trim();
                    publishTime = $_1('div.text-cm.text-gray').first().text().trim();
...
function saveToCSV(articles, keyword) {
    ...
    var csvPath = path.join(directory, "".concat(keyword.replace(/ /g, '_'), "_cnbc_articles.csv"));
