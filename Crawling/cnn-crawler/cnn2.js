...
var maxPages = 10000; // Maksimum halaman
...
function scrapeArticlesFromTagPage(keyword) {
    return __awaiter(this, void 0, void 0, function () {
        var pageNumber, morePages, articles, url, data, $, articleElements, i, element, title, link, articleUrl, articleData, $$, content, publishTime, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    pageNumber = 1;
                    ...
                    _a.label = 1;
                case 1:
                    if (!(morePages && pageNumber <= maxPages)) return [3 /*break*/, 10];
                    _a.label = 2;
                case 2:
                    _a.trys.push([2, 8, , 9]);
                    url = "https://www.cnnindonesia.com/tag/".concat(encodeURIComponent(keyword), "/").concat(pageNumber);
                ...
                case 3:
                    ...
                    _a.label = 4;
                case 4:
                    if (!(i < articleElements.length)) return [3 /*break*/, 7];
                    element = articleElements[i];
                    title = $(element).find('h2').text().trim();
                    link = $(element).find('a').attr('href') || '';
                    if (!(title && link)) return [3 /*break*/, 6];
                ...
                case 8:
                    error_1 = _a.sent();
                    console.error("Error scraping CNN for keyword \"".concat(keyword, "\" on page ").concat(pageNumber, ": ").concat(error_1));
                    morePages = false;
                    return [3 /*break*/, 9];
...
function saveToCSV(articles, keyword) {
    ...
    var csvPath = path.join(__dirname, "".concat(keyword.replace(/ /g, '_'), "part_2_scraped_articles.csv"));
    ...
    csv.writeRecords(articles)
        .then(function () {
        console.log("Artikel telah berhasil disimpan ke ".concat(keyword.replace(/ /g, '_'), "_CNN-Indonesia_articles.csv"));
    })
